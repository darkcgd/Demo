 ...
 *
 */
public class MyRevealLayout extends RelativeLayout {
    ...
    public MyRevealLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void setPositionInAdapter() {
        if (rippleInAdapter) {
            positionInAdapter = findParentAdapterView().getPositionForView(MyRevealLayout.this);
        }
    }

    private boolean adapterPositionChanged() {
        if (rippleInAdapter) {
            int newPosition = findParentAdapterView().getPositionForView(MyRevealLayout.this);
     ...
     * Animations
     */
    private Property<MyRevealLayout, Float> radiusProperty
            = new Property<MyRevealLayout, Float>(Float.class, "radius") {
        @Override
        public Float get(MyRevealLayout object) {
            return object.getRadius();
        }

        @Override
        public void set(MyRevealLayout object, Float value) {
    ...
    private Property<MyRevealLayout, Integer> circleAlphaProperty
            = new Property<MyRevealLayout, Integer>(Integer.class, "rippleAlpha") {
        @Override
        public Integer get(MyRevealLayout object) {
            return object.getRippleAlpha();
        }

        @Override
        public void set(MyRevealLayout object, Integer value) {
    ...
    * Accessor
    ...
    private class PerformClickEvent implements Runnable {
        ...
        private void clickAdapterView(AdapterView parent) {
            final int position = parent.getPositionForView(MyRevealLayout.this);
            final long itemId = parent.getAdapter() != null
                    ? parent.getAdapter().getItemId(position)
                    : 0;
            if (position != AdapterView.INVALID_POSITION) {
                parent.performItemClick(MyRevealLayout.this, position, itemId);
            }
        }
    }
    ...
    public static class RippleBuilder {
        ...
        public MyRevealLayout create() {
            MyRevealLayout layout = new MyRevealLayout(context);
            ...
            if (parent != null && parent instanceof MyRevealLayout) {
    ...
    /**
     * 设置水波纹效果,一般是首页底部的按钮,不跳转界面
     * @param view 为该view设置
     */
    public static void setAttributes(View view){
    	on(view).rippleColor(Color.parseColor("#A3A3A3"))   //设置波纹的颜色  设置颜色必须这样设置，通过R.color.XXXX的设置是无效的
		.rippleAlpha(0.1f).rippleHover(true).create();
    }
    public  void setAttributesNew(View view){
    	on(view).rippleColor(Color.parseColor("#083a6d"))   //设置波纹的颜色  设置颜色必须这样设置，通过R.color.XXXX的设置是无效的
		.rippleAlpha(1.0f).rippleHover(true).rippleDuration(400).create();
    }
    /**
     * 设置水波纹效果,一般是layout,跳转界面
     * @param view 为该view设置
     */
    public static void setLayoutAttributes(View view){
    	on(view).rippleColor(Color.parseColor("#A3A3A3"))   //设置波纹的颜色  设置颜色必须这样设置，通过R.color.XXXX的设置是无效的
    	.rippleAlpha(0.1f).rippleHover(true).rippleDuration(300).rippleDelayClick(true).create();
    }
